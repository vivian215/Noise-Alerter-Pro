...
package com.example.noisealerterpro.ThirdParty;


import android.util.Log;

import com.example.noisealerterpro.AppData;
...
// this file is modified for my project

public class MFCC {

    private final static int       n_mfcc       		= AppData.FEATURE_DIMENSION;
    ...
    public double[][] getMelSpectroGram()
    {
        return specTroGram;
    }
    ...
    public float[] process(double[] doubleInputBuffer) {
        mfccResult = dctMfcc(doubleInputBuffer);
//        return finalshape(mfccResult);
        return finalshapeMean(mfccResult);
    }

    //MFCC into 1d
    private float[] finalshapeMean(double[][] mfccSpecTro){
        float[] finalMfcc = new float[mfccSpecTro.length];
        int k = 0;
        Log.i("tflite", Integer.toString(mfccSpecTro[0].length) + " X " + Integer.toString(mfccSpecTro.length));
        for (int i = 0; i < mfccSpecTro.length; i++){
            double tmp = 0;
            for (int j = 0; j < mfccSpecTro[0].length; j++){
                tmp += mfccSpecTro[i][j];
            }
            finalMfcc[k] = (float)(tmp / mfccSpecTro[0].length);
            k = k+1;
        }
        return finalMfcc;
    }
...
//        final double[][] specTroGram = powerToDb(melSpectrogram(y));
        specTroGram = powerToDb(melSpectrogram(y));
